package com.aistudio.tuntivelho.leimaus.data.network

import android.util.Log
import com.aistudio.tuntivelho.leimaus.data.db.StampDao
import com.aistudio.tuntivelho.leimaus.data.db.StampEntity
import com.aistudio.tuntivelho.leimaus.data.preferences.DEFAULT_URL
import com.aistudio.tuntivelho.leimaus.data.preferences.FALLBACK_GRAPHQL_URL
...
class TuntivelhoRepository(
    ...
    private suspend fun executePunch(type: String): StampResult = withContext(Dispatchers.IO) {
        ...
        // Always log in first to establish PHP session cookies and fresh Bearer token (matching tuntiwelho_api.py)
        ...
        for (targetUrl in candidateUrls) {
            ...
            if (com.aistudio.tuntivelho.leimaus.BuildConfig.DEBUG) {
            ...
            try {
                ...
                if (code in 200..303) {
                    ...
                    if (leimaErrors?.isNotEmpty() == true) {
                        ...
                        Log.e("Tuntivelho", "Leima errors: $errMsg")
                    ...
                    if (topErrors?.isNotEmpty() == true) {
                        ...
                        Log.e("Tuntivelho", "Top-level GraphQL errors: $errMsg")
                        recordLog(now, fullTimeStr, actionTitle, isSuccess = false, message = errMsg)
                        return@withContext StampResult.Error(errMsg)
                    }
                    ...
                    if (leimaPayload == null) {
                        ...
                        Log.e("Tuntivelho", errMsg)
                        recordLog(now, fullTimeStr, actionTitle, isSuccess = false, message = errMsg)
                        return@withContext StampResult.Error("Palvelin ei palauttanut leimauksen vahvistusta.")
                    }

                    if (com.aistudio.tuntivelho.leimaus.BuildConfig.DEBUG) {
                        Log.d("Tuntivelho", "Stamp successful! PreviousStamp: ${leimaPayload.previousstamp}")
                    }
    ...
    private fun fetchKellokorttiDefaults(targetUrl: String, token: String): com.aistudio.tuntivelho.leimaus.data.network.SelectionDefaults? {
        ...
        return try {
            ...
            Log.e("Tuntivelho", "Error fetching defaults: ${e.localizedMessage}")
            null
        }
    }
    ...
    private fun fetchBalance(targetUrl: String, token: String): Long? {
        ...
        return try {
            ...
            Log.e("Tuntivelho", "Error fetching balance: ${e.localizedMessage}")
            null
        }
    }
