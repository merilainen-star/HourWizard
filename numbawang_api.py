"""
Numbawang Auto-Puncher
=======================
Automates punch-in / punch-out for the Tuntivelho (Finago Mobiili) timecard app.

Usage:
  python numbawang_api.py --username USER --password PASS --action punch_in
  python numbawang_api.py --username USER --password PASS --action punch_out
  python numbawang_api.py --username USER --password PASS --action test_login
  python numbawang_api.py --username USER --password PASS --action punch_in --dry-run

Requirements: Python 3 (stdlib only — no pip packages needed).

Environment variables:
  NW_USER / NW_PASS  -> credentials
  NW_API_URL (TW_API_URL is deprecated)         -> optional GraphQL endpoint override
"""
...
# The Tuntivelho PHP backend uses a session cookie (Tyovuorovelho=...)
...
def get_api_urls():
    ...
    env_url = (os.environ.get("NW_API_URL") or os.environ.get("TW_API_URL"))
    urls = []
    if env_url:
...
def main():
    parser = argparse.ArgumentParser(description="Numbawang Auto-Puncher")
    parser.add_argument("--username", default=(os.environ.get("NW_USER") or os.environ.get("TW_USER")),
                        help="Tuntivelho username (or set NW_USER env var (TW_USER is deprecated))")
    parser.add_argument("--password", default=(os.environ.get("NW_PASS") or os.environ.get("TW_PASS")),
                        help="Tuntivelho password (or set NW_PASS env var (TW_PASS is deprecated))")
    ...
    if not args.username or not args.password:
        parser.error("Credentials required: use --username/--password or set NW_USER/NW_PASS env vars")
